...
class Navigator():
    ...
    def google_login_start_server(self) -> None:
        ...
        time.sleep(0.5) # let the page load (use this to avoid a long wait in the try statement)
        ...
        try: 
            ...
            self.page.locator('#profile-item-small-instance-with-conda-store-ui').click()
    ...
    def reset_workspace(self):
        ...
        try:
            shut_down_all = self.page.get_by_text("Shut Down All Kernels...", exact=True)
        ...
        self.page.get_by_title(f"/home/{google_email}", exact=True).locator("path").click()
    ...
    def _set_environment_via_popup(self, kernel=None):
        ...
        if kernel is None:
            ...
            try:
                ...
                self.page.locator("div").filter(has_text="No KernelSelect").get_by_role("button", name="No Kernel").wait_for(timeout=300, state='attached')
        else:
            # set the environment
            self.page.get_by_role("combobox").nth(1).select_option(f"{{\"name\":\"{kernel}\"}}")
            ...
            try:
                ...
                self.page.locator("div").filter(has_text="No KernelSelect").get_by_role("button", name="Select").click()
    ...
    def set_environment(self, kernel):
        ...
        if not popup:
            ...
            self.page.get_by_role("menuitem", name="Change Kernel…").get_by_text("Change Kernel…").click()
...
class RunNotebook(Navigator):
    ...
    def run_notebook(self, path, expected_output_text):
        ...
        file_locator.wait_for(timeout=300000, state='attached') # 5 minutes max for server to spin up
        file_locator.click()
        self.page.get_by_role("menuitem", name="Open from Path…").get_by_text("Open from Path…").click()
        ...
        self.page.get_by_text("Kernel", exact=True).click()
        self.page.get_by_role("menuitem", name="Restart Kernel and Run All Cells…").get_by_text("Restart Kernel and Run All Cells…").click()
