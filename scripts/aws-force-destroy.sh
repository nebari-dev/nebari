...
mapfile -t volume_names < <(aws ec2 describe-volumes --filters "Name=tag:kubernetes.io/cluster/${PROJECT_NAME}-${NAMESPACE},Values=owned" --query 'Volumes[?Tags[?Key==`Name`]].VolumeId'  --output text)
