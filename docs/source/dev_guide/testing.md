...
balancer IP locally and set the domain to `--domain localhost`. Ports
`10080` and `10443` are arbitrary but if they are less than `1000`
require `root` permissions.

```shell
ssh -L 10080:<load-balancer-ip>:80 -L 10443:<load-balancer-ip>:443 <remote-host-ip>
```
...
access QHub via the load balancer IP by visiting it and you should get
...
The `qhub develop` command does not automate cleanup. This is
intentional to ensure that development iterations can take advantage
of caching of builds. When you are ready to delete your Minikube
cluster list all of the available profiles. The end of the `develop`
command output also shows the appropriate command to run.

```shell
/tmp/minikube/*/minikube profile list
```

This should list one or many profiles. Make sure to run `minikube
delete --profile <profile>`. By default `qhub develop` will create a
`qhub` profile.

When using the `--pr` and `--rev` options a `git worktree` is
used. All of the `git worktree`s can be listed via:

```shell
$ git worktree list
/home/username/p/quansight/qhub          7499b88f [minikube-provider]
/run/user/1000/qhub/qhub-develop-pr-885  7499b88f [qhub-develop-pr-885]
```

To remove a specific worktree run `git worktree remove <path>
--force`. Do not run the command on the main repository directory (in
this case `/home/username/p/quansight/qhub`). It is not critical
however that you delete these directories they only take up space and
will be erased when you turn of your machine. Read [git
worktree](https://git-scm.com/docs/git-worktree) for more information.
...
Docker-build will be tested. Use `qhub develop` mentioned above to
fully test the images as they are fully built with the command and
exposed in a local QHub cluster.
