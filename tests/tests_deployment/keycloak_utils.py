...
def get_keycloak_admin():
    ...
    config = read_configuration("nebari-config.yaml", config_schema)
    return get_keycloak_admin_from_config(config)


def create_keycloak_client_role(client_id: str, role_name: str, scopes: str, component: str):
    keycloak_admin = get_keycloak_admin()
...
def assign_keycloak_client_role_to_user(username: str, client_name: str, role: dict):
    ...
    keycloak_admin = get_keycloak_admin()
    user_details = get_keycloak_user_details_by_name(username=username, keycloak_admin=keycloak_admin)
    client_details = get_keycloak_client_details_by_name(client_name=client_name, keycloak_admin=keycloak_admin)
...
def create_keycloak_role(client_name: str, role_name: str, scopes: str, component: str):
    ...
    keycloak_admin = get_keycloak_admin()
    client_details = get_keycloak_client_details_by_name(client_name=client_name, keycloak_admin=keycloak_admin)
    return create_keycloak_client_role(client_details["id"], role_name=role_name, scopes=scopes, component=component)
