    ...
    def get_user_groups(self):
        ...
        for groupname in need_gids:
            ...
            keycloak_admin.update_group(
                groups[groupname]["id"],
                {"attributes": {"gid": [str(max_gid)]}, "name": groupname},
            )
        ...
        for username in need_uids:
            ...
            keycloak_admin.update_user(
                users[username]["id"], {"attributes": {"uid": [str(max_uid)]}}
            )
...
keycloak_store = KeycloakStore(
    KEYCLOAK_SERVER_URL, KEYCLOAK_USERNAME, KEYCLOAK_PASSWORD, KEYCLOAK_REALM_NAME
)
...
async def getetcpasswd():
    ...
    for _, u in HOLDALL.users.items():
        ...
        print(u)
        lines.append(
            f"{u.username}:x:{u.uid}:{gid}:{u.username}:/home/jovyan:/bin/bash"
        )
    return "\n".join(lines)
